    ...
    static void bfs(){
        ...
        while(!q.isEmpty()){
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if(map[nx][ny] == 0){ // 이동할 수 있는곳이면
                    // 이미 왔던 곳이면 pass
                    if (visited[nx][ny][now.breakWall])  continue;
                    q.offer(new Node(nx, ny, now.cnt + 1, now.breakWall));
                    visited[nx][ny][now.breakWall] = true;
                }
                else{ // 벽을 부수고 가야하는 상황이면
                    // 이미 벽을 부순적 있거나, 방문했던 적 있는 곳이면 pass
                    if(now.breakWall != 0 || visited[nx][ny][now.breakWall + 1]) continue;
